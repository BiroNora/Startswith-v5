...
export async function POST({ request }) {
	try {
		...
		// 3. A KOMPLEX LEKÉRDEZÉS - KIJAVÍTVA
    const schoolsData = await db.$queryRaw<any[]>`
            ...
            EventInterests AS (
                SELECT
                    i.event_id,
                    SUM(CASE WHEN i.status = 0 THEN i.intrest_count ELSE 0 END) AS s0,
                    SUM(CASE WHEN i.status = 1 THEN i.intrest_count ELSE 0 END) AS s1,
                    SUM(CASE WHEN i.status = 2 THEN i.intrest_count ELSE 0 END) AS s2,
                    SUM(CASE WHEN i.status = 3 THEN i.intrest_count ELSE 0 END) AS s3,
                    SUM(CASE WHEN i.applied = TRUE THEN i.intrest_count ELSE 0 END) AS appl
                FROM interested i
                GROUP BY i.event_id
            ),
