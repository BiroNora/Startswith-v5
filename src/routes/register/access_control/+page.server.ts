...
export const load: PageServerLoad = async ({ locals }) => {
  if (!locals.user || (!locals.user.isDirector && !locals.user.isSuperior)) throw redirect(302, '/auth/login');
...
export const actions: Actions = {
  user_active_change: async ({ request, locals }) => {
    ...
    if (!hasPermission) return fail(403, { unauthorized: true, message: "Nincs jogosultsága a felhasználó módosításához." });
